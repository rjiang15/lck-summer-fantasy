...
export default function DraftBoard({
  ...
  playersPerRole, groupKeys, groups, order, teams, availablePlayers,
}: {
  leagueId: number; status: string; currentPick: number; totalPicks: number; currentTeamId: number | null;
  budget: number; uniformPrice: number; pricingMode: string; priceSource: string | null; playersPerRole: number;
  ...
  const [selectedRole, setSelectedRole] = useState<DraftRole>("Top");
  ...
  const rolePlayers = availablePlayers.filter((player) => player.role === selectedRole && (selectedGroup === "ALL" || player.group === selectedGroup));
  ...
  return <>
    <section className="card draft-status">
      <div>
        <span className="draft-kicker">Snake draft · {pricingMode === "DYNAMIC" ? "dynamic prices" : "uniform prices"}</span>
    ...
    {status === "ACTIVE" && <section>
      <div className="section-heading"><div><h2>Available players</h2><p className="muted small">A drafted player disappears immediately. Every pick must preserve a safe group, role, and budget path for all fantasy teams.</p></div><span className="badge pending">{availablePlayers.length} available</span></div>
      ...
      <div className="draft-player-grid">{rolePlayers.map((player) => {
        ...
        const reserve = slotAvailable ? minimumDraftCompletionCost([...currentComposition, candidate], remainingPool, playersPerRole, groupKeys) : null;
        ...
        if (currentTeamIndex >= 0) everyTeamComposition[currentTeamIndex] = [...currentComposition, candidate];
        const preservesLeaguePool = currentTeamIndex >= 0 && draftPoolSupportsAllTeams(everyTeamComposition, remainingPool, playersPerRole, groupKeys);
        const conservativeReserve = preservesLeaguePool ? conservativeDraftCompletionCost(currentTeamIndex, everyTeamComposition, remainingPool, playersPerRole, groupKeys) : null;
        const affordable = reserve !== null && conservativeReserve !== null && currentSpent + player.price + conservativeReserve <= budget;
