...
test("dynamic prices are monotonic, bounded, stepped, and average exactly $1,000", () => {
  const result = calculateDynamicPrices([
    { playerId: "best", ppg: 30, games: 20 },
    { playerId: "high", ppg: 24, games: 20 },
    { playerId: "middle", ppg: 18, games: 20 },
    { playerId: "low", ppg: 12, games: 20 },
    { playerId: "lowest", ppg: 6, games: 20 },
  ...
  assert.equal(result.players.new.price, DYNAMIC_PRICE_AVERAGE);
});
